...
class CopyableLog(TextArea):
    """Read-only TextArea that auto-copies selected text to clipboard on mouse up."""
    ...
    """
    CLIP_FILE = Path("/tmp/wolf-copy.txt")
    ...
    def on_mouse_up(self, event: Any) -> None:
        self.call_after_refresh(self._copy_if_selected)

    def _copy_if_selected(self) -> None:
        text = self.selected_text
        if not text:
            return
        # Write to temp file (works everywhere)
        try:
            self.CLIP_FILE.write_text(text)
        except OSError:
            pass
        # Also try OSC 52 for terminals that support it
        try:
...
class _LegacyWerewolfApp(App[None]):
    BINDINGS = [
    ...
    selected_player_id: reactive[int | None] = reactive(None)
    ...
    def _configure_players_table(self) -> None:
        players = self.query_one("#players", DataTable)
        players.clear(columns=True)
        players.cursor_type = "row"
        players.zebra_stripes = True
        players.add_columns(
            self._i18n.t("column.seat"),
            self._i18n.t("column.role"),
            self._i18n.t("column.faction"),
            self._i18n.t("column.status"),
            self._i18n.t("column.sheriff"),
            self._i18n.t("column.death"),
        )
    ...
    def action_toggle_language(self) -> None:
        ...
        if self._view is not None:
            self._render_players_table(self._view)
    ...
    def action_start_game(self) -> None:
        ...
        self._update_statusbar(extra=self._i18n.t("status.launching"))
    ...
    def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
        if event.data_table.id != "players":
            return
        try:
            self.selected_player_id = int(str(event.row_key.value))
        except ValueError:
    ...
    def _reload_view(self, *, force: bool = False) -> None:
        try:
            view = load_game_view(self._database, game_id=self.game_id, event_scope=self.event_scope, event_limit=240)
        except sqlite3.DatabaseError:
            view = None
        if view is None:
            self.title = self._i18n.t("app.title")
            self.query_one("#statusbar", Static).update(self._i18n.t("empty.no_games"))
        ...
        self._render_players_table(view)
        self._render_events(view, force=force)
        self._render_inspector()
        self._update_statusbar()

    def _render_players_table(self, view: dict[str, Any]) -> None:
        players = self.query_one("#players", DataTable)
        players.clear(columns=False)
        for row in view["players"]:
            alive = bool(row["survived"])
            role_text = self._i18n.enum("role", row["role"])
            if not alive:
                role_text = "".join(c + STRIKETHROUGH for c in role_text) + " ☠"
            players.add_row(
                str(row["seat_index"]),
                role_text,
                self._i18n.enum("faction", row["faction"]),
                self._i18n.t("value.alive") if alive else self._i18n.t("value.dead"),
                self._i18n.t("value.yes") if row["is_sheriff"] else "",
                self._translate_optional("death", row["death_cause"]),
                key=str(row["player_id"]),
            )
        if self.selected_player_id is None and view["players"]:
            self.selected_player_id = int(view["players"][0]["player_id"])
        if self.selected_player_id is not None:
            try:
                index = [int(row["player_id"]) for row in view["players"]].index(self.selected_player_id)
                players.move_cursor(row=index, column=0)
            except Exception:
                pass
    ...
    def _render_events(self, view: dict[str, Any], *, force: bool = False) -> None:
        ...
        if not same_stream:
            ...
            for event in view["events"][len(self._event_seqs) :]:
                log.write(self._format_event(event))
    ...
    def _render_inspector(self) -> None:
        ...
        lines = [
            self._i18n.t("panel.game"),
            f"{self._i18n.t('panel.id')}: {game['id'][:8]}",
            f"{self._i18n.t('panel.state')}: {self._i18n.enum('status', game['status'])}",
        ]
        # Winner + reason
        winner = game.get("winner")
        if winner:
            winner_text = self._translate_optional("winner", winner)
            ...
            if reason_text:
                lines.append(f"{self._i18n.t('panel.winner')}: {winner_text} — {reason_text}")
            else:
                lines.append(f"{self._i18n.t('panel.winner')}: {winner_text}")
        else:
            lines.append(f"{self._i18n.t('panel.winner')}: -")
        lines.append(f"{self._i18n.t('panel.round')}: {game['round_count']}")
        lines.append(f"{self._i18n.t('panel.config')}: {game['config_id']}")
        lines.append(f"{self._i18n.t('panel.phase')}: {self._translate_optional('phase', snapshot['phase'] if snapshot else None)}")
        lines.append("")
        lines.append(self._i18n.t("panel.metrics"))
        lines.append(f"{self._i18n.t('panel.events')}: {metrics['total_events'] if metrics else '-'}")
        lines.append(f"{self._i18n.t('panel.llm')}: {metrics['total_llm_calls'] if metrics else '-'}")
        lines.append(f"{self._i18n.t('panel.fallbacks')}: {metrics['total_fallbacks'] if metrics else '-'}")
        lines.append("")
        lines.append(self._i18n.t("panel.player"))
        if selected is None:
            lines.append(self._i18n.t("panel.none"))
        else:
            lines.extend(
                [
                    f"{self._i18n.t('field.seat')}: {selected['seat_index']}",
                    f"{self._i18n.t('field.role')}: {self._i18n.enum('role', selected['role'])}",
                    f"{self._i18n.t('field.faction')}: {self._i18n.enum('faction', selected['faction'])}",
                    f"{self._i18n.t('field.status')}: "
                    f"{self._i18n.t('value.alive') if selected['survived'] else self._i18n.t('value.dead')}",
                    f"{self._i18n.t('field.sheriff')}: "
                    f"{self._i18n.t('value.yes') if selected['is_sheriff'] else self._i18n.t('value.no')}",
                    f"{self._i18n.t('field.death')}: {self._translate_optional('death', selected['death_cause'])}",
                ]
            )
        inspector.update("\n".join(lines))

    def _selected_player(self) -> dict[str, Any] | None:
        if self._view is None or self.selected_player_id is None:
            return None
        return next(
            (row for row in self._view["players"] if int(row["player_id"]) == self.selected_player_id),
            None,
        )

    def _update_statusbar(self) -> None:
        self._update_statusbar_impl()

    def _update_statusbar_impl(self, extra: str | None = None) -> None:
        ...
        worker_state = ""
        if self._launch_worker is not None and self._launch_worker.state == WorkerState.RUNNING:
            worker_state = "  worker=running"
        if extra:
            worker_state = f"  {extra}"
        status.update(
            f"{self._i18n.t('status.game')} {game_short} ({game_pos})  "
            f"{self._i18n.t('status.scope')} {self._i18n.enum('scope', self.event_scope)}  "
            f"{self._i18n.t('status.auto')} "
            f"{self._i18n.t('status.on') if self.auto_refresh else self._i18n.t('status.off')}  "
            f"{self._i18n.t('status.selected')} {self.selected_player_id or '-'}  "
            f"Lang {self._language}{worker_state}"
        )
    ...
    def _format_event(self, event: dict[str, Any]) -> Text:
        ...
        text = Text()
        text.append(f"{event['seq']:03d} ", style="dim")
        text.append(f"R{event['round']} {self._translate_optional('phase', event['phase'])} ", style="bold")
        text.append(f"{self._i18n.enum('scope', scope)} ", style=color)
        text.append(f"{self._translate_optional('event', event['event_type'])}: ", style="bold")
        text.append(self._event_message(event))
        return text
    ...
    def _event_message(self, event: dict[str, Any]) -> str:
        ...
        if content_key.startswith("event."):
            ...
            if detail:
                return self._format_event_detail_template(detail, data)
        return str(event.get("content") or "")

    def _format_i18n(self, key: str, **kwargs: object) -> str:
        return self._i18n.t(key).format(**kwargs)

    def _format_event_detail_template(self, template: str, data: dict[str, Any]) -> str:
        values: dict[str, object] = dict(data)
        player_id = data.get("player_id") or data.get("actor_id") or data.get("speaker_id")
        target_id = data.get("target_id")
        if player_id is not None:
            values.setdefault("player", self._format_player(player_id))
            values.setdefault("actor", self._format_player(player_id))
        if target_id is not None:
            values.setdefault("target", self._format_player(target_id))
        try:
    ...
    def _format_player_list(self, values: object) -> str:
        if not isinstance(values, list):
            return "-"
        return "、".join(self._format_player(value) for value in values) if self._language == "zh" else ", ".join(
            self._format_player(value) for value in values
        )
    ...
    def _format_votes(self, votes: object) -> str:
        ...
        return "，".join(parts) if self._language == "zh" else ", ".join(parts)
